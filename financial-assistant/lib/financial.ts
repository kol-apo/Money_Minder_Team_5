...
export async function getFinancialSummary(userId: string): Promise<FinancialSummary> {
  try {
    const summaries = await query("SELECT * FROM financial_summaries WHERE user_id = ?", [userId])
...
export async function getSavingsGoals(userId: string): Promise<SavingsGoal[]> {
  try {
    const goals = await query("SELECT * FROM savings_goals WHERE user_id = ? ORDER BY deadline ASC", [userId])
...
export async function addSavingsGoal(
  ...
  try {
    ...
    await query("INSERT INTO savings_goals (id, user_id, name, target, current, deadline) VALUES (?, ?, ?, ?, 0, ?)", [
      goalId,
      userId,
      name,
      target,
...
export async function contributeToSavingsGoal(goalId: string, userId: string, amount: number): Promise<boolean> {
  try {
    ...
    const goals = await query("SELECT * FROM savings_goals WHERE id = ? AND user_id = ?", [goalId, userId])
    ...
    // Update the goal
    await query("UPDATE savings_goals SET current = ? WHERE id = ? AND user_id = ?", [newAmount, goalId, userId])
